...
public class PlayerController : MonoBehaviour
{
    ...
    {   
        if (ammo.Count > 0)//Fix
        {
            ...
            ammo.Remove(ammo[0]);
            fruitShot.GetComponent<Rigidbody>().AddForce(transform.forward * fruitShootingForce, ForceMode.Force); // forward should be something tweakable to try different shooting angles
            timer = 0;
        }
        else { return; }
